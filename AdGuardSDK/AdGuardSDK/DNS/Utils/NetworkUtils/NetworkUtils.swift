...
public protocol NetworkUtilsProtocol {
    ...
    func getProtocol(from upstream: String, _ implementation: DnsImplementation) throws -> DnsProtocol
    ...
    func getCurrentNetworkStateSync() -> NetworkStateInfo
}
...
public class NetworkUtils: NetworkUtilsProtocol {
    ...
    public init() {
        ...
        monitor().pathUpdateHandler = { newPath in
            self.logNWPath(newPath)

            let internetState: InternetState = switch newPath.status {
                case .satisfied: .available
                case .unsatisfied: .unavailable
                case .requiresConnection: .unavailable
                @unknown default: .unavailable
            }

            NotificationCenter.default.post(name: .networkStateChanged, object: NetworkStateInfo(internetState, self.getInterface(newPath.availableInterfaces)))
    ...
    public var systemDnsServers: [String] {
        if let interfaceName = getCurrentNetworkInterfaceSync()?.name {
            return ["system://\(interfaceName)"]
        }
    ...
    public func getProtocol(from upstream: String, _ implementation: DnsImplementation) throws -> DnsProtocol {
        if upstream.hasPrefix("sdns://") {
            return try getDnsProtocol(from: upstream)
        }

        if upstream.hasPrefix("https://") || (upstream.hasPrefix("h3://") && implementation == .adGuard) {
            return .doh
        } else if upstream.hasPrefix("tls://") {
            return .dot
        } else if upstream.hasPrefix("quic://") {
            return .doq
        } else if (checkUpstreamByPrefix(upstream, "udp://") || checkUpstreamByPrefix(upstream, "tcp://")) && implementation == .adGuard {
            return .dns
        } else {
    ...
    public func getCurrentNetworkInterfaceSync() -> NWInterface? { getInterface(monitor().currentPath.availableInterfaces) }

    public func getCurrentNetworkStateSync() -> NetworkStateInfo {
        let path = monitor().currentPath
        let internetState: InternetState = switch path.status {
            case .satisfied: .available
            case .unsatisfied: .unavailable
            case .requiresConnection: .unavailable
            @unknown default: .unavailable
        ...
        return NetworkStateInfo(internetState, getInterface(path.availableInterfaces))
    }
    ...
    private func checkUpstreamByPrefix(_ upstream: String, _ prefix: String) -> Bool {
        if upstream.hasPrefix(prefix), let range = upstream.range(of: prefix) {
            let candidate = String(upstream[range.upperBound...])
            return UrlUtils.isIpv4(candidate) || UrlUtils.isIpv6(candidate)
        }
    ...
    private func getInterface(_ interfaces: [NWInterface]) -> NWInterface? {
        if let ipsecInterface = interfaces.first(where: { $0.name.contains("ipsec") }) {
            return ipsecInterface
        }
...
public class NetworkStateChanged {
    public let stateInfo: NetworkStateInfo

    public init(_ stateInfo: NetworkStateInfo) { self.stateInfo = stateInfo }
}
